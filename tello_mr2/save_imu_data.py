...
class TelloAccTester(Node):
    ...
    def dump_data_to_csv(self, out_dir: str):
        now = datetime.now()
        dt_string = now.strftime("%d-%m-%Y_%H-%M-%S")
        filename = f'tello_acc_data_{dt_string}.csv'
        ...
        buffer_df.to_csv(f'{out_dir}/{filename}')
...
def main(args=None):
    ...
    tello_acc_tester = TelloAccTester()

    out_dir = 'tello_drift_tests_mocap_05-05-22/hanging_tello_2_2'
    # out_dir = 'tello_drift_optitrack/tello_drift_ot_moving'

    trajectory_hanging = [(0, 'land'), (10, 'land')]
    ...
    tello_acc_tester.dump_data_to_csv(out_dir)
